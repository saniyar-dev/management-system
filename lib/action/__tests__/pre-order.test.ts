...
import { UpdatePreOrder, DeletePreOrder, CheckPreOrderDependencies } from '../pre-order';
...
describe('Pre-Order CRUD Operations', () => {
  ...
  describe('UpdatePreOrder', () => {
    it('should successfully update a pre-order', async () => {
      ...
      const mockSupabaseChain = {
        single: vi.fn().mockResolvedValue({ data: mockPreOrderData, error: null }),
    ...
    it('should fail when trying invalid status transition', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.message).toContain('تغییر وضعیت از converted به pending مجاز نیست');
    });
    ...
    it('should fail when trying to convert non-approved pre-order', async () => {
      ...
      expect(result.message).toContain('تغییر وضعیت از pending به converted مجاز نیست');
    });
  ...
  describe('DeletePreOrder', () => {
    ...
    it('should fail to delete pre-order with dependencies', async () => {
      ...
      const mockDependencySupabaseChain = {
        limit: vi.fn().mockResolvedValue({ data: [{ id: 'order-1' }], error: null }),
        eq: vi.fn().mockReturnThis(),
      };
      ...
      expect(result.message).toBe('این پیش سفارش به سفارش تبدیل شده و قابل حذف نیست.');
    });
