...
describe('Hierarchical Permission System', () => {
  ...
  it('should respect permission hierarchy', async () => {
    ...
    try {
      ...
      const foundRestrictedClient = data?.find(client => client.id === restrictedClient.id);
      expect(foundRestrictedClient).toBeUndefined();

      // Test Layer 4 user (mask = 8) accessing the same client
      const { data: layer4Data, error: layer4Error } = await supabase.rpc('get_filtered_clients_with_permissions', {
        requesting_user_id: testPanelUserId,
      ...
      const foundByLayer4 = layer4Data?.find(client => client.id === restrictedClient.id);
  ...
  describe('Count Function Tests', () => {
    it('should return correct count with permission filtering', async () => {
      ...
      const { data: countData, error: countError } = await supabase.rpc('get_filtered_clients_total_with_permissions', {
        requesting_user_mask: 2, // Layer 2 user
        _types: ['all'],
        _statuses: ['all'],
      });
      ...
      const { data: clientsData, error: clientsError } = await supabase.rpc('get_filtered_clients_with_permissions', {
        requesting_user_id: testPanelUserId,
        requesting_user_mask: 2,
        _types: ['all'],
        _statuses: ['all'],
    ...
    it('should return 0 for invalid permission mask', async () => {
      // Test with invalid permission mask
      const { data, error } = await supabase.rpc('get_filtered_clients_total_with_permissions', {
        requesting_user_mask: 16, // Invalid mask
        _types: ['all'],
        _statuses: ['all'],
      });
    ...
    it('should respect type and status filtering in count', async () => {
      ...
      try {
        ...
        const { data: personCount, error: personCountError } = await supabase.rpc('get_filtered_clients_total_with_permissions', {
          requesting_user_mask: 2,
          _types: ['person'],
          _statuses: ['all'],
        });
        ...
        const { data: activeCount, error: activeCountError } = await supabase.rpc('get_filtered_clients_total_with_permissions', {
          requesting_user_mask: 2,
          _types: ['all'],
          _statuses: ['active'],
        });
        ...
        const { data: specificCount, error: specificCountError } = await supabase.rpc('get_filtered_clients_total_with_permissions', {
          requesting_user_mask: 2,
          _types: ['person'],
          _statuses: ['active'],
        });
        ...
        const { data: specificClients, error: specificClientsError } = await supabase.rpc('get_filtered_clients_with_permissions', {
          requesting_user_id: testPanelUserId,
          requesting_user_mask: 2,
          _types: ['person'],
          _statuses: ['active'],
    ...
    it('should handle permission hierarchy correctly in count', async () => {
      ...
      try {
        ...
        for (const userLevel of userLevels) {
          // Get count
          const { data: count, error: countError } = await supabase.rpc('get_filtered_clients_total_with_permissions', {
          ...
          // Get actual data
          const { data: clients, error: clientsError } = await supabase.rpc('get_filtered_clients_with_permissions', {
            requesting_user_id: testPanelUserId,
            requesting_user_mask: userLevel.mask,
            _types: ['all'],
            _statuses: ['all'],
          ...
          if (clients) {
            const accessibleMasks = clients.map(c => c.permission_mask).sort();
            const expectedMasks = userLevel.expectedAccess.filter(mask =>
              (userLevel.mask & mask) === userLevel.mask
            ).sort();
