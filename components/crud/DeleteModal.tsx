...
interface DeleteModalProps<T extends RowData, S extends string>
  extends DeleteComponentProps<T, S> {
  isOpen: boolean;
  onClose: () => void;
  title: string;
...
export function DeleteModal<T extends RowData, S extends string>({
  ...
  React.useEffect(() => {
    if (isOpen && dependencyCheck) {
      ...
      dependencyCheck(entity.id.toString())
        .then((hasDependencies) => {
          if (hasDependencies) {
            setDependencyError(
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-warning-50 border border-warning-200 rounded-lg p-4">
                    <p className="text-warning-800 text-sm text-right">
                      آیا از حذف این {entityDisplayName} اطمینان دارید؟ این عمل
                      قابل بازگشت نیست.
                    </p>
                  </div>
                        ...
                        <div className="flex justify-between items-start py-2">
                          ...
                          <div className="text-sm text-default-900 text-right flex-1 mr-4">
                            <Chip
                              ...
                              color={
                                statusColorMap?.[entity.status] || "default"
                              }
                              size="sm"
                              variant="flat"
                            >
